...
def close_appointment_window():
    global appointment_id  # Add this line
    appointment_id = None  # Reset appointment_id when closing the appointment window
...
def show_appointment_summary(appointment_info):
    ...
    def update_appointment():
        ...
        global appointment_id
        appointment_id = appointment_info.get("AppointmentId", None)
        ...
        for i, (key, value) in enumerate(appointment_info.items()):
        ...
        def save_updated_appointment(appointment_id):
            update_values = {key: entry.get() if hasattr(entry, 'get') else entry for key, entry in updated_info.items()}

            update_query = "UPDATE appointments SET age=%s, contact_number=%s, sex=%s, reason=%s, disease=%s, doctor=%s, appointment_time=%s, appointment_date=%s, amount=%s WHERE AppointmentId=%s"

            update_values = (
                int(update_values["Age"]),
                str(update_values["Contact Number"]),
                str(update_values["Sex"]),
                str(update_values["Reason"]),
                str(update_values["Disease"]),
                str(update_values["Doctor"]),
                str(update_values["Appointment Time"]),
                str(update_values["Appointment Date"]),
                str(update_values["Amount"]),
                int(appointment_id)  # Use AppointmentId in the update query
            )
        ...
        save_button = tk.Button(update_window, text="Save Update", command=lambda aid=appointment_id: save_updated_appointment(aid))
        save_button.grid(row=len(updated_info), columnspan=2, pady=10)

        cancel_button = tk.Button(update_window, text="Cancel", command=update_window.destroy)
        cancel_button.grid(row=len(updated_info) + 1, columnspan=2, pady=10)

    summary_window = tk.Toplevel(root)
    summary_window.geometry("500x700")
    summary_window.title("Appointment Summary")

    for i, (key, value) in enumerate(appointment_info.items()):
        if key == "Appointment Date":
            formatted_date = value.split()[0]
            label = tk.Label(summary_window, text=f"{key}:", font=("Helvetica", 12))
        else:
            label = tk.Label(summary_window, text=f"{key}:", font=("Helvetica", 12, "bold"))

        label.grid(row=i, column=0, pady=5, padx=10, sticky=tk.W)

        detail_label = tk.Label(summary_window, text=f"{value}", font=("Helvetica", 12))
        detail_label.grid(row=i, column=1, pady=5, padx=10, sticky=tk.W)

    button_width = 25
    button_height = 1

    update_appointment_button = tk.Button(summary_window, text="Update Appointment", bg='lightblue', command=update_appointment, width=button_width, height=button_height)
    update_appointment_button.grid(row=len(appointment_info), column=0, pady=20, padx=10, sticky=tk.W)

    print_summary_button = tk.Button(summary_window, text="Print Summary and Save to File", bg='lightblue', command=lambda: save_summary_to_file(appointment_info), width=button_width, height=button_height)
    print_summary_button.grid(row=len(appointment_info), column=1, pady=20, padx=10, sticky=tk.W)

    # Update the cancel button in your code to call the 'cancel_appointment' function
    cancel_button = tk.Button(summary_window, text="Cancel Appointment", command=lambda: cancel_appointment(appointment_info.get("AppointmentId")), bg='lightcoral', fg='black', width=button_width, height=button_height)
    cancel_button.grid(row=len(appointment_info) + 1, column=0, pady=20, padx=10, sticky=tk.W)

    # Create a "Close All" button
    close_all_button = tk.Button(summary_window, text="Close All", command=stop_system, bg='lightgreen', fg='black', width=button_width, height=button_height)
    close_all_button.grid(row=len(appointment_info) + 1, column=1, pady=20, padx=10, sticky=tk.W)
...
def cancel_appointment(appointment_id):
    ...
    global summary_window  # Declare summary_window as global

    # Assuming 'appointment_id' is the unique identifier for the appointment in your database

    # Create a delete query
    delete_query = "DELETE FROM appointments WHERE AppointmentId=%s"
    ...
    try:
        ...
        messagebox.showinfo("Appointment Cancelled", "Appointment cancelled successfully!")
