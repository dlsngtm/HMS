...
def create_search_window():
    ...
    search_window.title("Search Available Doctors")

    search_date_label = tk.Label(search_window, text="Search Date:")
    search_date_label.pack()
    search_date_entry = Calendar(search_window, selectmode="day", year=2023, month=1, day=1)
    search_date_entry.pack()

    search_time_label = tk.Label(search_window, text="Search Time:")
    search_time_label.pack()
    search_time_var = tk.StringVar(value="9:00 AM - 10:00 AM")  # Set a default value
    search_time_entry = tk.Entry(search_window, textvariable=search_time_var)
    search_time_entry.pack()

    search_doctor_label = tk.Label(search_window, text="Search Doctor:")
    search_doctor_label.pack()
    doctors = list(doctor_appointments.keys())
    search_doctor_var = tk.StringVar(value=doctors[0])  # Set the default doctor
    search_doctor_option_menu = tk.OptionMenu(search_window, search_doctor_var, *doctors)
    search_doctor_option_menu.pack()

    search_button = tk.Button(search_window, text="Search", command=search_available_doctors)
    search_button.pack()

    def search_available_doctors():
        global search_date_entry, search_time_var, search_doctor_var
        search_date = search_date_entry.get_date()
        search_time = search_time_var.get()
        search_doctor = search_doctor_var.get()

        # Perform the search and display the available doctors
        available_doctors = get_available_doctors(search_doctor, search_date, search_time)

        # Display the available doctors in a messagebox or a new window
        if available_doctors:
            message = f"Available doctors for {search_date} at {search_time}:\n\n"
            message += "\n".join(available_doctors)
            messagebox.showinfo("Available Doctors", message)
        else:
            messagebox.showinfo("No Available Doctors", "No doctors available for the selected date and time.")


def get_available_doctors(search_doctor, search_date, search_time):
    available_doctors = []
    try:
        # Check if the search_date is in the doctor_appointments dictionary
        if search_date in doctor_appointments[search_doctor]:
            # Check if the search_time is available for the selected doctor and date
            if search_time not in doctor_appointments[search_doctor][search_date]:
                available_doctors.append(search_doctor)
    except KeyError:
        pass  # Handle the case when the search_doctor or search_date is not in the dictionary

    return available_doctors

# Create a button to open the search window
search_button = tk.Button(root, text="Search Available Doctors", command=create_search_window)
...
def appointment_window(full_name, age, sex):
    ...
    # Add a new button for opening the search window
    search_button = tk.Button(appointment_window, text="Search Available Doctors", command=create_search_window)
    search_button.pack()
...
search_button = tk.Button(root, text="Search Available Doctors", command=open_search_window)
search_button.pack(pady=10)
