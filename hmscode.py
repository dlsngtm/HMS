...
# Initialize doctor_appointments based on existing data in the database
def initialize_doctor_appointments():
    ...
    try:
        query = "SELECT doctor, appointment_time FROM appointments"
        ...
        for appointment in appointments:
            ...
            if doctor not in doctor_appointments:
                doctor_appointments[doctor] = [slot]
            else:
                doctor_appointments[doctor].append(slot)
    except Exception as e:
...
def save_appointment_to_database(appointment_info):
    try:
        ...
        slot = appointment_info["Appointment Time"]

        # Query the database for existing appointments for the selected doctor and slot
        query_existing = "SELECT COUNT(*) FROM appointments WHERE doctor = %s AND appointment_time = %s"
        cursor.execute(query_existing, (doctor, slot))
        existing_count = cursor.fetchone()[0]

        if existing_count >= MAX_APPOINTMENTS_PER_SLOT:
            print("Doctor has reached the appointment limit for this slot.")
            messagebox.showerror("Appointment Limit Exceeded", "The doctor has reached the maximum appointment limit for this slot.")
            return False
