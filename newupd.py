...
def initialize_doctor_appointments():
    ...
    try:
        query = "SELECT appointmentid, doctor, appointment_time, appointment_date FROM appointments"
...
def delete_appointment(appointment_id, cursor, connection):
    try:
        if connection:
            query = f"DELETE FROM appointments WHERE AppointmentId = {appointment_id}"
            cursor.execute(query)
            connection.commit()
...
def appointment_window(full_name, age, sex):
    ...
    def schedule_appointment():
        ...
        if patient_name and patient_age and contact_number and formatted_appointment_date:
            appointment_info = {
                "Name": full_name,
                "Age": age,
            ...
            if save_appointment_to_database(appointment_info):
                appointments[full_name] = appointment_info
                # Show appointment summary
                show_appointment_summary(appointment_info)
                appointment_window.destroy()  # Close the appointment scheduling window
        else:
...
def show_appointment_summary(appointment_info):
    def update_appointment():
        ...
        def save_updated_appointment(appointment_info):
            ...
            if 'Appointment ID' in appointment_info:
                update_values = (
                    ...
                    str(update_dict["Amount"]),  # Assuming amount is a string
                )

                # Update the database with the modified information
                update_query = "UPDATE `appointments` SET `AppointmentId`=%s,`Patient_Name`=%s,`Age`=%s,`Contact_Number`=%s,`Sex`=%s,`Reason`=%s,`Doctor`=%s,`Appointment_Time`=%s,`Appointment_Date`=%s,`Amount`=%s,`Disease`=%s WHERE `AppointmentId`=%s"
        ...
        save_updated_appointment_button = tk.Button(update_window, text="Save Updated Appointment", command=lambda: save_updated_appointment(appointment_info), bg="green", fg="white", font=("Arial", 12))
    ...
    cancel_button = tk.Button(summary_window, text="Cancel Appointment", command=lambda: cancel_appointment(appointment_info, summary_window), bg="red", fg="white", font=("Arial", 12))
