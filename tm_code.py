...
def save_appointment_to_database():
    ...
    if patient_name and age and contact_number and appointment_date:
        ...
        try:
            ...
            query = "INSERT INTO doctorappointment (patient_name, age, contact_number, sex, reason, doctor, appointment_time, appointment_date, amount) " \
...
def schedule_appointment():
    ...
    if patient_name and age and contact_number and appointment_date:
        ...
        # Prepare the appointment information to save
        appointment_info = {
            "Name": patient_name,
            "Age": age,
    ...
    def schedule_appointment():
        ...
        if patient_name and age and contact_number and appointment_date:
            ...
            appointment_info = {
                ...
                "Doctor": selected_doctor,
                "Appointment Date": appointment_date,
                "Appointment Time": selected_slot,  # Add selected time
                "Reason": selected_reason,
...
def show_appointment_summary(appointment_info):
    ...
    insert_query = "INSERT INTO appointments (appointment_date, other_column, ...) VALUES (%s, %s, ...)"
    cursor.execute(insert_query, (formatted_date, appointment_info["Other Column"], ...))
...
def search_appointments():
    ...
    query = "SELECT * FROM doctorappointment WHERE 1"
    if search_date:
        query += f" AND appointment_date = '{search_date}'"
    if search_time:
        query += f" AND appointment_time = '{search_time}'"
    if search_doctor:
        query += f" AND doctor = '{search_doctor}'"
...
def show_search_results(results):
    ...
    search_results_tree = ttk.Treeview(search_results_window, columns=("Name", "Date", "Time", "Doctor", "Reason"))
    search_results_tree.heading("#1", text="Name")
    search_results_tree.heading("#2", text="Date")
    search_results_tree.heading("#3", text="Time")
    search_results_tree.heading("#4", text="Doctor")
    ...
    for result in results:
        search_results_tree.insert("", "end", values=(result[1], result[3], result[4], result[5], result[6]))
