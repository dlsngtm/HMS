...
def show_appointment_summary(appointment_info):
    def update_appointment():
        ...
        def save_updated_appointment():
            ...
            update_query = "UPDATE appointments SET patient_name=%s, age=%s, contact_number=%s, sex=%s, reason=%s, disease=%s, doctor=%s, appointment_time=%s, appointment_date=%s, amount=%s WHERE patient_name=%s"
            ...
            update_values = (
                ...
                str(update_values["Amount"]),  # Assuming amount is a float
        ...
        # Additional code for updating the appointment
        save_updated_appointment_button = tk.Button(update_window, text="Save Updated Appointment", command=save_updated_appointment)
        save_updated_appointment_button.grid(row=len(update_window.grid_slaves()) + 1, column=1, pady=10)

        # Create a "Print PDF" button
        print_pdf_button = tk.Button(update_window, text="Print PDF", command=lambda: print_to_pdf(updated_info))
        print_pdf_button.grid(row=len(update_window.grid_slaves()) + 2, column=1, pady=10)
    ...
    cancel_button = tk.Button(summary_window, text="Cancel Appointment", command=cancel_appointment)
    cancel_button.pack()

    # Create a "Close All" button
    close_all_button = tk.Button(summary_window, text="Close All", command=stop_system)
    close_all_button.pack()
...
def cancel_appointment():
    ...
    if result == "yes":
        ...
        appointment_id = appointment_info["Appointment ID"]
